...
class HarmonicDivergence(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Average True Range for 14 periods: measures volatility for the window
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
        ...
        # Retrieve the entry candle information
        ...
        stoploss = trade_candle.loc[resample('low')] - trade_candle.loc[resample('atr')]
        print(f"Pair: {pair}, current profit: {current_profit}")
        if (stoploss < current_rate) and (current_profit <= 0.0):
            return (stoploss / current_rate) - 1
        elif (current_profit > 0) and (current_profit < 0.015):
            return -1  # keep using the current stoploss
...
def initialize_divergences_lists(dataframe: DataFrame):
    dataframe["total_bullish_divergences"] = np.full(len(dataframe['close']), np.nan)
    dataframe["total_bullish_divergences_count"] = np.full(len(dataframe['close']), 0)
    dataframe["total_bullish_divergences_names"] = np.full(len(dataframe['close']), '')
    dataframe["total_bearish_divergences"] = np.full(len(dataframe['close']), np.nan)
    dataframe["total_bearish_divergences_count"] = np.full(len(dataframe['close']), 0)
    dataframe["total_bearish_divergences_names"] = np.full(len(dataframe['close']), '')
...
def divergence_finder_dataframe(dataframe: DataFrame, indicator_source: str) -> Tuple[pd.Series, pd.Series]:
    bearish_lines = [np.full(len(dataframe['close']), np.nan)]
    bearish_divergences = np.full(len(dataframe['close']), np.nan)
    bullish_lines = [np.full(len(dataframe['close']), np.nan)]
    bullish_divergences = np.full(len(dataframe['close']), np.nan)
    low_iterator = []
    high_iterator = []
